import os
from InsurancePremium.entity.config_entity import TRANSFORMER_OBJECT_FILE_NAME \
    , TARGET_ENCODER_OBJECT_FILE_NAME, MODEL_FILE_NAME
...
class ModelResolver:
    ...
    def get_lates_save_dir_path(self)->str:
        try:
            latest_dir=self.get_latest_dir_path()
            if latest_dir==None:
                return os.path.join(self.model_registry,f"{0}")
            latest_dir_num=int(os.path.basename(self.get_lates_save_dir_path()))
    ...
    def get_latest_save_model_path(self):
        try:
            latest_dir=self.get_lates_save_dir_path()
